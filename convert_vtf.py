...
def convert_images_to_vtf(input_dir, output_dir, vtfcmd_path, texture_size="2048x2048", callback=None):
    ...
    for file in os.listdir(input_dir):
        ...
        # Normal map flag & format de compression haute qualité / lossless
        if is_normal_map:
            # On ne doit PAS utiliser "-normal" car il agit comme un générateur de normal map depuis une heightmap.
            # Au lieu de cela, on spécifie le flag NORMAL pour le moteur de jeu, et on choisit BGR888 (ou BGRA8888 avec alpha)
            # pour éviter les artefacts destructeurs de la compression DXT sur le relief.
            cmd_args += ["-flag", "NORMAL", "-format", "BGR888", "-alphaformat", "BGRA8888"]
        ...
        try:
            ...
            converted_files += 1
            converted_vtfs.append(os.path.splitext(file)[0] + ".vtf")
    ...
    return converted_files, converted_vtfs
...
def convert_vtf(model_name, chemin_modeles=None, chemin_materials=None, texture_size="2048x2048", callback=None):
    ...
    converted_files, _ = convert_images_to_vtf(input_dir, output_dir, vtfcmd_path, texture_size, callback)
