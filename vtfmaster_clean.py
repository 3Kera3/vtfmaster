...
def _resolve_to_root(path: str, target_name: str) -> str:
    """
    Remonte dans le chemin donné jusqu'à trouver un dossier nommé `target_name`
    et retourne ce chemin tronqué.

    Exemples :
      ".../garrysmod/materials/models/kera"  -> ".../garrysmod/materials"
      ".../garrysmod/models/kera"            -> ".../garrysmod/models"
      ".../garrysmod/materials"              -> ".../garrysmod/materials"  (déjà bon)
      ".../garrysmod/models"                 -> ".../garrysmod/models"     (déjà bon)

    Si `target_name` n'est pas trouvé dans le chemin, retourne le chemin tel quel.
    """
    p = Path(path)
    parts = p.parts
...
class VTFMasterGUI:
    ...
    def charger_configuration(self):
        ...
        default_config = {
            "source1_base": r"C:/Program Files (x86)/Steam/steamapps/common/GarrysMod/garrysmod/materials",
            "source2_base": r"C:/Program Files (x86)/Steam/steamapps/common/GarrysMod/garrysmod/models",
    ...
    def setup_vtf_tab(self):
        ...
        self.var_reprocess_all_vmt = tk.BooleanVar(value=False)
        self.var_nocull = tk.BooleanVar(value=False)
        self.var_alphatest = tk.BooleanVar(value=False)
        self.var_translucent = tk.BooleanVar(value=False)
        ...
        ttk.Checkbutton(options_frame, text=tr(self.language, "reprocess_all_vmt"), variable=self.var_reprocess_all_vmt).grid(
        ...
        self.texture_size_combo.grid(row=0, column=2, padx=5, sticky="w")

        ttk.Label(options_frame, text=tr(self.language, "preset_label")).grid(row=1, column=1, sticky="e", padx=10)
        self.vmt_preset_display_var = tk.StringVar(value=tr(self.language, "preset_props"))
        self.vmt_preset_combo = ttk.Combobox(
            options_frame,
            textvariable=self.vmt_preset_display_var,
            values=[tr(self.language, "preset_props"), tr(self.language, "preset_map_hammer")],
            state="readonly",
            width=30,
        )
        self.vmt_preset_combo.grid(row=1, column=2, padx=5, sticky="w")

        # Options VMT (nocull, alphatest, translucent)
        ttk.Label(options_frame, text=tr(self.language, "vmt_options")).grid(row=2, column=1, sticky="e", padx=10)
        
        vmt_opts_subframe = ttk.Frame(options_frame)
        vmt_opts_subframe.grid(row=2, column=2, padx=5, sticky="w")
        
        ttk.Checkbutton(vmt_opts_subframe, text=tr(self.language, "opt_nocull"), variable=self.var_nocull).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Checkbutton(vmt_opts_subframe, text=tr(self.language, "opt_alphatest"), variable=self.var_alphatest).pack(side=tk.LEFT, padx=(0, 10))
        ttk.Checkbutton(vmt_opts_subframe, text=tr(self.language, "opt_translucent"), variable=self.var_translucent).pack(side=tk.LEFT)
    ...
    def get_selected_preset_name(self) -> str:
        val = self.vmt_preset_display_var.get()
        if val == tr(self.language, "preset_map_hammer"):
            return "map_hammer"
        return "props"
    ...
    def _run_conversion_worker(self):
        try:
            ...
            if do_vtf:
                ...
                converted_count, converted_vtfs = convert_images_to_vtf(
                ...
                self.log(f"[OK] VTF: {converted_count} fichier(s) converti(s).")
            ...
            if do_vmt:
                ...
                # Si "Repasser sur tous les VMT existants" est coché, on traite tous les VTF
                reprocess_all = bool(self.var_reprocess_all_vmt.get())
                files_to_process = None if reprocess_all else converted_vtfs

                if not do_vtf or reprocess_all:
                ...
                preset = self.get_selected_preset_name()
                vmt_count = process_vtf_folder(
                    dest_dir,
                    vmt_rel,
                    vtf_files_to_process=files_to_process,
                    preset_name=preset,
                    nocull=bool(self.var_nocull.get()),
                    alphatest=bool(self.var_alphatest.get()),
    ...
    def setup_vmt_edit_tab(self):
        ...
        # Selection de preset
        select_frame = ttk.Frame(self.vmt_edit_frame)
        select_frame.pack(fill=tk.X, padx=15, pady=5)

        ttk.Label(select_frame, text=tr(self.language, "vmt_edit_preset_select")).pack(side=tk.LEFT, padx=(0, 10))
        
        self.vmt_edit_preset_var = tk.StringVar(value=tr(self.language, "preset_props"))
        self.vmt_edit_preset_combo = ttk.Combobox(
            select_frame,
            textvariable=self.vmt_edit_preset_var,
            values=[tr(self.language, "preset_props"), tr(self.language, "preset_map_hammer")],
            state="readonly",
            width=30,
        )
        self.vmt_edit_preset_combo.pack(side=tk.LEFT)
        self.vmt_edit_preset_combo.bind("<<ComboboxSelected>>", self.on_vmt_edit_preset_changed)
    ...
    def get_selected_edit_preset_name(self) -> str:
        val = self.vmt_edit_preset_var.get()
        if val == tr(self.language, "preset_map_hammer"):
            return "map_hammer"
        return "props"

    def on_vmt_edit_preset_changed(self, event=None):
        self.load_vmt_preset_into_ui()

    def load_vmt_preset_into_ui(self):
        try:
            from vmt_template import load_vmt_template
            preset_name = self.get_selected_edit_preset_name()
            template = load_vmt_template(preset_name=preset_name, base_dir=get_base_dir())
        except Exception as e:
            self.log(f"[WARN] Erreur chargement preset: {e}")
            try:
                from vmt_template import DEFAULT_TEMPLATES, DEFAULT_PROPS_TEMPLATE
                preset_name = self.get_selected_edit_preset_name()
                template = DEFAULT_TEMPLATES.get(preset_name, DEFAULT_PROPS_TEMPLATE)
            except Exception:
                template = ""
    ...
    def save_vmt_preset(self):
        ...
        try:
            ...
            preset_name = self.get_selected_edit_preset_name()
            save_vmt_template(txt, preset_name=preset_name, base_dir=get_base_dir())
    ...
    def reset_vmt_preset(self):
        try:
            ...
            preset_name = self.get_selected_edit_preset_name()
            reset_vmt_template(preset_name=preset_name, base_dir=get_base_dir())
            self.load_vmt_preset_into_ui()
    ...
    def setup_addon_tab(self):
        ...
        src_mat = _resolve_to_root(self.source1_base, "materials")
        src_mod = _resolve_to_root(self.source2_base, "models")
        chemins_text = f"Source materials : {src_mat}\nSource models : {src_mod}\nDossier Workshop : {self.workshop_base}"
